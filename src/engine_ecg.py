    ...
    def _evaluate_one_epoch(self, epoch, mode, train=True):
        ...
        with torch.no_grad():
            ...
            for i in iterator:
                ...
                output = torch.mean(output_dict["ecg_out"], dim=0)
                # print(output_dict["x"])
                ...
                if torch.sum(data_dict["ecg_concat_mask"]) > 0 and self.config.data.add_ecg:
                    loss = self.criterion['sample_weighted_bce'](output_dict["ecg_out"].squeeze(1), data_dict["label"], data_dict["ecg_concat_mask"]) ###############
