...
class Engine:
    ...
    def _build(self):
        ...
        self.dataloaders, self.data_dirs = data_builder_value.build(
            self.config.data,
            self.train,
            self.transform,
            self.aug_transform,
        ...
        with torch.no_grad():
            ...
            if self.train and mode!='test':
                ...
                if self.config.train.evaluator.maximize:
                    ...
                    if avg_loss < self.best_eval_metric:
                        self.save_checkpoint(epoch=epoch, eval_metric=eval_metric, save=True)
                    self.best_eval_metric = (
                        avg_loss
                        if avg_loss < self.best_eval_metric
                        else self.best_eval_metric
    ...
    def _forward_path(self, data_dict, epoch):
        ...
        if self.config.data.lge_value:
            ...
            # print(output_dict["lge_value"])
            # print(data_dict["lge_value"])
            loss = loss + self.criterion['regression'](output_dict["lge_value"], data_dict["lge_value"]) ##############
            if epoch % 10 == 0:
