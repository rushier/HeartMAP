...
def plot_attn_with_ecg(
    ...
    attn_weights = attn_weights[index].cpu().detach().numpy()
    ecg = ecg[index].cpu().detach().numpy()
    v, aug_attn_weights = attention_rollout(attn_weights)
    v = np.sum(v, axis=0)[np.newaxis,:]
    v = (v-np.amin(v))/(np.amax(v)-np.amin(v))
    # v = scipy.special.softmax(v, axis=1)
    v = skimage.transform.resize(v, (v.shape[0],len(ecg)), 1) 
    

    # 可视化
    # for pulse_index in range(len(label)):
    plt.figure(figsize=(20, 3))

    # 绘制热图
    ax1 = plt.gca()
    sns.heatmap(v, cmap="OrRd", vmin=0, vmax=1, ax=ax1)
    plt.xlabel("Timestamp")
    plt.ylabel("Amplitude")

    # 绘制滤波后的信号波形
    ax2 = ax1.twinx()
    ax2.plot(ecg, color='black')
    ...
    ax1.set_xticklabels([0,1000,2000,3000,4000], rotation=0)
    ax2.set_yticks([])

    # 显示图表
    plt.tight_layout()
    # plt.savefig(f'/data_153/wangchengxiang/wcx_pulse2/a_final_experiment/interpretability/{name_list[pulse_index]}.png', dpi = 500)
    # plt.show()
    ...
    os.makedirs(save_path, exist_ok=True)
    plt.savefig(
        os.path.join(save_path, "{}_ecg_attn_{}_{}.png".format(mode, epoch, iteration))
    )
    plt.close("all")
...
def plot_all_attn_maps(
    ...
    if patch_attn is not None:
        if test:
            ...
            if 1:
                image_idx = 8
                print(vids.shape, patch_attn.detach().cpu().numpy().shape)
                fig, ax = plt.subplots(2, len(vids))
                for view_num in range(len(vids)):
                    result, save_path = plot_attn_with_img(
                        epoch=epoch,
                        iteration=iteration,
                        use_wandb=use_wandb,
                        save_dir=save_dir,
                        img=vids[view_num,image_idx,:,:,0].detach().cpu().numpy(),
                        attn_weights=patch_attn.detach().cpu().numpy()[view_num*16+image_idx],
                        log_str="patch",
                        step_metric={"step": step},
                        mode=mode,
                    ...
                    ax[0,view_num].imshow(vids[view_num,image_idx,:,:,0].detach().cpu().numpy(), cmap="gray")
                    ax[1,view_num].imshow(result, cmap="jet")
            ...
            plot_attn_with_img(
                ...
                use_wandb=use_wandb,
                save_dir=save_dir,
                img_all=vids[0, 0, 0, :, :, 0].detach().cpu().numpy(),
            ...
            # 每一个batch的第一个样本的第一个view的第一帧
            # attn的绘制方式与是否有cls token有什么关系
