...
from . import fine_grained_lge_value
...
class SpaceTimeFactorizedViViT(nn.Module):
    def __init__(
        ...
        if mode == "ef":
            ...
            self.global_value_output_mlp = nn.Sequential(
                nn.Linear(vid_hidden_size, vid_hidden_size // 2),
                nn.ReLU(inplace=True),
                nn.Dropout(p=output_dropout_rate),
                nn.Linear(vid_hidden_size // 2, vid_hidden_size // 4),
    ...
    def forward(self, data_dict):
        ...
        lge_value, lge_loc = self.gnn_model(patches, patch_mask)
        lge_value = F.sigmoid(lge_value) * 100
        ...
        if self.output_mlp is not None:
            ...
            global_value = self.global_value_output_mlp(x)
