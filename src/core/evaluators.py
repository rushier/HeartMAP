import numpy as np
from sklearn.metrics import r2_score, accuracy_score, f1_score, balanced_accuracy_score, roc_auc_score
...
class F1ScoreEvaluator(object):
    ...
    def compute(self) -> float:
        ...
        try:
            self.score = f1_score(
                y_true=self.y_true, y_pred=self.y_pred > self.threshold
            )
        except:
            self.score = -1
...
class AUCEvaluator(object):
    ...
    def compute(self) -> float:
        ...
        """
        # 在数据加载的地方添加检查
        # print(f"y_true 数据类型: {type(self.y_true)}")
        # print(f"y_true 形状: {self.y_true.shape if hasattr(self.y_true, 'shape') else 'N/A'}")
        # print(f"y_true 唯一值: {np.unique(self.y_true)}")
        # print(f"y_true NaN值位置: {np.where(np.isnan(self.y_true))[0]}")

        # print(f"y_pred 数据类型: {type(self.y_pred)}")
        # print(f"y_pred 形状: {self.y_pred.shape if hasattr(self.y_pred, 'shape') else 'N/A'}")
        # print(f"y_pred 范围: {np.min(self.y_pred)} - {np.max(self.y_pred)}")
        # print(f"y_pred NaN值位置: {np.where(np.isnan(self.y_pred))[0]}")
        try:
...
class AccuracyEvaluator(object):
    ...
    def compute(self) -> float:
        ...
        :return: r2 score
        """
        self.accuracy = accuracy_score(y_true=self.y_true, y_pred=self.y_pred>self.threshold)
