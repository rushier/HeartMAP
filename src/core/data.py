...
class HuaxiLGEDataset(Dataset):
    def __init__(
        ...
        self.all_chamber_info = pd.read_csv('./csv_files/input_all_chamber_videos.csv')
    ...
    def __getitem__(self, item):
        ...
        global_value = row['total_enhanced']
        if '%' in str(global_value):
            global_value = global_value.split('%')[0]
        try:
            ...
            for chamber in self.chamber_list:
                if isinstance(all_chamber_videos_row[chamber], str):
                    ...
                    while not find:
                        ...
                        seg_files = glob('../data/save_processed/EchoLGE_multi_task/' + '/*/*/' + '_'.join(path.split('/'))+'_*_*.npz')
                        if len(seg_files) != 0:
                ...
                if chamber != 'mri' and chamber != 'ecg' :
                    if not isinstance(path, str) or 'not exists' in path:
                        complete_view = False
                    if not self.args.add_flow:
                        array, mask = self.read_chamber(path, chamber, root_path, row)
                    ...
                    path = row[chamber]
                    path = path.replace('./data_transfer/CMR/', '../data/HuaxiCMR_processed/')
        ...
        return_dict =  {
            ...
            "global_value":torch.tensor(global_value).type(torch.FloatTensor),
        ...
        for key, value in return_dict.items():
            if isinstance(value, torch.Tensor) and torch.isnan(value).any():
                print(f"[NaN Warning] `{key}` contains NaN values in sample `{PID}_{echodate}`.")
                if key == "vid":
                    print("vid shape:", value.shape)
                    print("vid NaN indices:", torch.isnan(value).nonzero())
                elif key == "lge_value":
                    print("LGE value:", value)
                elif key == "global_value":
                    print("Global value:", value)
                elif key == "label":
                    print("Label:", value)
                else:
                    print(f"{key} value summary: min={value.min()}, max={value.max()}")
        return return_dict
    ...
    def read_chamber(self, ch_path, chamber, root_path, row):
        if not isinstance(ch_path, str) or 'not exists' in ch_path:
            have_zero = True
            image_array = np.zeros((self.length,self.size, self.size))
            mask = np.zeros((self.length,self.size, self.size))
            ...
            if not self.train:
                # selected_row = self.selected_cycles[self.selected_cycles['PID'] == row['PID']]
                # selected_row = selected_row[selected_row['Echodate'] == int(row['Echodate'])].iloc[0]
                selected_row = row
                save_processed_file = selected_row[chamber+'_input'].replace('../../../', '../')
                print('selected', save_processed_file)
            else:
                seg_files = glob('../data/save_processed/EchoLGE_multi_task/' + '/*/*/' + '_'.join(ch_path.split('/'))+'_*_*.npz')
                if len(seg_files) == 0:
                    print('../data/save_processed/EchoLGE_multi_task/' + '/*/*/' + '_'.join(ch_path.split('/'))+'_*_*.npz')
                try:
            ...
            if not os.path.exists(save_processed_file):
                ...
                # array = skimage.transform.resize(array, (self.length, self.args.frame_size, self.args.frame_size), 1)
                # mask = skimage.transform.resize(mask, (self.length, self.args.frame_size, self.args.frame_size), 0)
                if np.isnan(image_array).all():
                    print(f"1 [NaN Warning] `{key}` contains NaN values in sample `{image_path}`.")
                    return None
                if np.isnan(mask).all():
                    print(f"1 [NaN Warning] `{key}` mask contains NaN values in sample `{image_path}`.")
                    return None
                ...
                if image_array.shape[-1] != self.args.frame_size:
                    if np.isnan(image_array).all():
                        print(f"2 [NaN Warning] `{key}` contains NaN values in sample `{image_path}`.")
                        return None
                    if np.isnan(mask).all():
                        print(f"2 [NaN Warning] `{key}` mask contains NaN values in sample `{image_path}`.")
                        return None
    ...
    def read_chamber_flow(self, ch_path, chamber, root_path):
        if not isinstance(ch_path, str) :
            ...
            save_processed_file = flow_mask_file.replace(self.flow_path, './data_transfer/EchoLGE_flow/')
            if not os.path.exists(save_processed_file):
                if not os.path.exists('/'.join(save_processed_file.split('/')[:-1])):
                    os.makedirs('/'.join(save_processed_file.split('/')[:-1]))
    ...
    def load_ecg(self, ecg_filename):
        ...
        if isinstance(ecg_filename, str) and 'not exists' not in ecg_filename:
            # ecg_filename = ecg_filename.replace('../data/HuaxiECG','./data_transfer/')
...
class HuaxiLGEDataset_external_test(Dataset):
        ...
        if self.args.chamber_list != 'ecg':  
            for chamber in self.chamber_list:
                ...
                if len(all_paths) > 0:
                    ...
                    middle_index = len(all_paths_sorted) // 2
                    path = all_paths_sorted[middle_index]
                    path = path.replace('\\','/')
                else:
                    path = False
                if chamber != 'mri' and chamber != 'ecg' :
    ...
    def read_chamber(self, ch_path, chamber, root_path, row):
        if not isinstance(ch_path, str) or not isinstance(row[chamber+'_input'], str) or row[chamber+'_input']=='None':
            have_zero = True
            image_array = np.zeros((self.length,self.size, self.size))
            mask = np.zeros((self.length,self.size, self.size))
            self.masked_attentions.append(np.zeros(self.length))
            ...
            if 1:
                if not self.train: 
                    if 1:
                        ...
                        # selected_row = selected_row[selected_row['Echodate'] == int(row['Echodate'])].iloc[0]
                        selected_row = row
                        save_processed_file = selected_row[chamber+'_input']
                        save_processed_file = save_processed_file.replace('\\', '/')
                        if 'ailab' in save_processed_file:
                            image_path = root_path + '/' + ch_path
                            save_processed_file = image_path.replace(root_path, '../data/save_processed/EchoLGE_multi_task/')
                            # new_image_path = image_path.replace(root_path, './data_transfer/cycles/DMD_周末随访_第二批/')
                            # save_processed_file = image_path.replace(root_path, './data_transfer/EchoLGE_multi_task/')
                            save_processed_file = save_processed_file.replace('.nii.gz', '.npz')
                            print('not selected', save_processed_file)
                        elif './data/save_cycles/Myocardial_Fibrosis/' in save_processed_file:
                            file = save_processed_file.split('GEMS_IMG')[-1]
                            try:
                                image_path = glob('../data/qq_cycles/DMD_MF_1112/*/*/' + file)[0]
                            except:
                                print('list index out of range', save_processed_file)
                            root_path = image_path.split('GEMS_IMG')[0]
                            save_processed_file = image_path.replace(root_path, '../data/save_processed/EchoLGE_multi_task/')
                            save_processed_file = save_processed_file.replace('.nii.gz', '.npz')
                        else:
                            file = selected_row[chamber+'_input'].replace('\\', '/')
                            if '../../../' in selected_row[chamber+'_input']:
                                save_processed_file = selected_row[chamber+'_input'].replace('../../../', '../')
                            else:
                                print(file)
                                image_path = glob('../data/qq_cycles/DMD_MF_1112/*/' + file)[0]
                                print('selected', save_processed_file)
                                save_processed_file = '../data/save_processed/EchoLGE_multi_task/' + file
                    # except:
                    #     image_path = root_path + '/' + ch_path
                    #     save_processed_file = image_path.replace(root_path, '../data/save_processed/EchoLGE_multi_task/')
                    #     # new_image_path = image_path.replace(root_path, './data_transfer/cycles/DMD_周末随访_第二批/')
                ...
                if not os.path.exists(save_processed_file):
                    if not os.path.exists('/'.join(save_processed_file.split('/')[:-1])):
                        os.makedirs('/'.join(save_processed_file.split('/')[:-1]))
                    print('saved', save_processed_file)
                    mask = np.ones((self.length,self.size, self.size))
                    print(image_path)
                    image_array = np.load(image_path)['array']
                    # array = skimage.transform.resize(array, (self.length, self.args.frame_size, self.args.frame_size), 1)
                    # mask = skimage.transform.resize(mask, (self.length, self.args.frame_size, self.args.frame_size), 0)
                    try:
                        ...
                        print(f"NaN values count: {np.isnan(image_array).sum()}")
                        print(f"Image array min/max: {np.nanmin(image_array)}, {np.nanmax(image_array)}")
                    mask = skimage.transform.resize(mask, (self.length, self.args.frame_size, self.args.frame_size), 0)
                    np.savez(save_processed_file, processed_img=image_array, processed_mask=mask)
                else:
                    if 1:
                        ...
                        mask = array['processed_mask']
                    # except:
                    #     print('error', save_processed_file)
                    if image_array.shape[-1] != self.args.frame_size:
                        image_array = skimage.transform.resize(image_array, (self.length, self.args.frame_size, self.args.frame_size), 1)
                        mask = skimage.transform.resize(mask, (self.length, self.args.frame_size, self.args.frame_size), 0)
    ...
    def compute_beat_based_attention_mask(self, ecg_data):
        ...
        return attention_mask
def read_mp4_with_opencv(video_path):
    """
    使用OpenCV读取MP4文件
    """
    # 创建视频捕获对象
    ...
    while True:
        ...
        #     print(f"已读取 {frame_number}/{frame_count} 帧")
    
    # 释放资源
    cap.release()
    
    # print(f"视频读取完成，共 {len(frames)} 帧")
    return frames, fps

class Fudan_test(Dataset):
    def __init__(
        self,
        args,
        info_csv,
    ...
    def __getitem__(self, item):
        ...
        if self.args.chamber_list != 'ecg':  
            for chamber in self.chamber_list:
                ...
                if not isinstance(path, str):
                    complete_view = False
                array = self.read_chamber(chamber, row)
                # (50, 600, 800, 3)
                all_arrays.append(array)
            all_arrays = np.array(all_arrays)
            self.masked_attentions = np.array(self.masked_attentions)
        ...
        ecg_data, ecg_mask, ecg_concat_mask = self.load_ecg()
        return {
            "vid": cine,
            "ecg_data":ecg_data,
            "ecg_mask":ecg_mask,
    ...
    def read_chamber(self, chamber, row):
        path = row[chamber]
        if not isinstance(path, str):
            have_zero = True
            image_array = np.zeros((self.length,self.size, self.size, 3))
            self.masked_attentions.append(np.zeros(self.length))
        else:
            number = row[chamber+'_备注']
            frames, fps = read_mp4_with_opencv(path)
            frames = np.array(frames)
            frames = frames[:,int((frames.shape)[1]/8):-20,int((frames.shape)[2]/6):-50, :]
            if len(frames) > 64:
                middle = len(frames)//2
                number = number > 1
                frames = frames[number*middle:(number+1)*middle]
            image_array = skimage.transform.resize(frames, (self.length, self.args.frame_size, self.args.frame_size, 3), 1)
            image_array = (image_array - np.amin(image_array)) / (np.amax(image_array)-np.amin(image_array))
            self.masked_attentions.append(np.ones(self.length))
        return image_array
...
def transform_echo_data_robust(df):
    """
    更稳健的版本，处理某些切面可能缺失数据的情况
    """
    # 创建唯一的check标识
    try:
        df['check'] = df['PID'].astype(str) + '_' + df['Echodate'].astype(str)
    except:
        print('缺少PID与Echodate')
        df['check'] = df['path'].apply(lambda x: x.split('/')[-2])
    
    result_rows = []
    unique_checks = df['check'].unique()
    print(len(unique_checks))
    
    for check in unique_checks:
        check_data = df[df['check'] == check].copy()
        
        # 获取所有存在的切面类型
        available_chambers = check_data['chamber'].unique()
        target_chambers = ['a2c', 'a3c', 'a4c', 'mid', 'mv']
        
        # 只处理实际存在的切面
        chambers_to_process = [chamber for chamber in target_chambers if chamber in available_chambers]
        
        if not chambers_to_process:
            continue
            
        chamber_data = {}
        for chamber in chambers_to_process:
            chamber_df = check_data[check_data['chamber'] == chamber].copy()
            chamber_data[chamber] = chamber_df
        
        # 收集每个切面的cycle=1数据
        cycle1_data = {}
        for chamber in chambers_to_process:
            chamber_df = chamber_data[chamber]
            try:
                cycle1_df = chamber_df[chamber_df['cycle_number'] == 1]
            except:
                print(cycle1_df)
            cycle1_data[chamber] = cycle1_df
        
        # 找出所有切面都有的共同路径（基于原始路径的相似性）
        # 这里我们简化处理，选择每个切面的前几个cycle=1的记录
        
        # 确定最小可用的cycle=1记录数
        min_cycle1_count = min([len(cycle1_data[chamber]) for chamber in chambers_to_process]) if chambers_to_process else 0
        
        if min_cycle1_count >= 3:
            # 如果所有切面都有至少3个cycle=1，选择前3个
            selected_count = 3
            selection_method = "first_3_cycle1"
        else:
            # 否则使用最小可用数（最多3个）
            selected_count = min(min_cycle1_count, 3)
            selection_method = f"available_cycle1_{selected_count}"
        
        # 为每个选中的记录创建行
        for i in range(selected_count):
            row_data = {'check': check}
            
            for chamber in target_chambers:
                path_col = f'{chamber}_path'
                cycle_col = f'{chamber}_cycle'
                
                if chamber in chambers_to_process and i < len(cycle1_data[chamber]):
                    # 选择该切面的第i个cycle=1记录
                    chamber_row = cycle1_data[chamber].iloc[i]
                    row_data[path_col] = chamber_row['path']
                    row_data[cycle_col] = chamber_row['cycle_number']
                else:
                    row_data[path_col] = None
                    row_data[cycle_col] = None
        ...
        if selected_count < 3:
            ...
            for chamber in chambers_to_process:
                chamber_df = chamber_data[chamber]
                # 排除已经选择的cycle=1数据
                already_selected = cycle1_data[chamber].iloc[:selected_count].index if selected_count > 0 else []
                available_df = chamber_df[~chamber_df.index.isin(already_selected)]
                all_available_data.append(available_df)
            
            # 合并所有可用数据
            if all_available_data:
                combined_available = pd.concat(all_available_data, ignore_index=True)
                
                if len(combined_available) >= remaining:
                    # 随机选择补充数据
                    supplementary_data = combined_available.sample(n=remaining, random_state=42)
                    
                    for _, sup_row in supplementary_data.iterrows():
                        row_data = {'check': check}
                        ...
                        for chamber in target_chambers:
                            ...
                            if chamber == sup_row['chamber']:
                                row_data[path_col] = sup_row['path']
                                row_data[cycle_col] = sup_row['cycle_number']
                            else:
                                # 对于其他切面，选择可用的数据
                                if chamber in chambers_to_process and len(chamber_data[chamber]) > 0:
                                    chamber_row = chamber_data[chamber].iloc[0]
                                    row_data[path_col] = chamber_row['path']
                                    row_data[cycle_col] = chamber_row['cycle_number']
                                else:
                                    row_data[path_col] = None
    ...
    # 确保列的顺序
    columns_order = ['check', 'a2c_path', 'a2c_cycle', 'a3c_path', 'a3c_cycle', 
                    'a4c_path', 'a4c_cycle', 'mid_path', 'mid_cycle', 'mv_path', 'mv_cycle']
...
class tumor_test(Dataset):
    def __init__(
        ...
        if split != 'train':
            self.dataset = selected_cycles
        if transform_df:
            self.dataset = transform_echo_data_robust(self.dataset)
        ...
        self.all_root_paths = ['../data/qq_cycles/raw_cycles_tumor_0901/tumor_0901/',
                  '../data/qq_cycles/raw_cycles_tumor_0822/',
                  '../data/qq_cycles/raw_cycles_wqh_0822/',
                  '../data/qq_cycles/raw_cycles_CLBBB_quality/',
                  '../data/qq_cycles/raw_cycles_HSCT/',
                  '../data/qq_cycles/raw_cycles_tumor_0525/',
                  '../data/qq_cycles/raw_cycles_tumor_0603_cycle_quality/',
                  '../data/qq_cycles/raw_cycles_normal_0525/',
                  '../data/qq_cycles/raw_cycles_fetus_SVA_handed/',
                  '../data/qq_cycles/raw_cycles_DMD_0603/',
                  '../data/qq_cycles/raw_cycles_BMD_0603_quality/',
                  './data/qq_cycles/raw_cycles_wqh/',
                  './data/qq_raw_cycles/',
                  './data/qq_cycles/raw_cycles_zll/',
                  '../data/qq_cycles/raw_cycles_tumor_0901/',
                  '../data/qq_cycles/raw_cycles_tumor_0901/tumor_0901/huaxi_add/',
                  '../data/qq_cycles/tumor/',
                  '../data/qq_cycles/']
    def class_samplers(self):
        labels_AS = self.dataset['LGE'].values.tolist()
        class_sample_count_AS = np.array(
    ...
    def read_chamber(self, chamber, row):
        ...
        try:
            ...
            if self.root_path is not None:
                ...
                for rp in self.all_root_paths:
                    cycle_num = str(int(row[chamber+'_cycle']))
                    #print(rp + '/' + path)
                    full_path = glob(rp + path+'_'+cycle_num + '_*.npz')
                    #print('full1', full_path, len(self.all_root_paths))
                    if len(full_path) != 0:
                        #print('full', full_path)
                        full_path = full_path[0]
                        break

            frames = np.load(full_path)['array']
            image_array = skimage.transform.resize(frames, (self.length, self.args.frame_size, self.args.frame_size, 3), 1)
            image_array = (image_array - np.amin(image_array)) / (np.amax(image_array)-np.amin(image_array))
            self.masked_attentions.append(np.ones(self.length))
