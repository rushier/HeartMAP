...
# from . import fine_grained_lge_value_new as fine_grained_lge_value ####################20251111改动
from . import fine_grained_lge_value ####################20251111改动
...
class HuBERTForECGClassification(nn.Module):
    ...
    def __init__(
        ...
        if use_label_embedding: # for classification only
            ...
            if classifier_hidden_size is None: # no hidden layer
                ...
                self.fc_1 = nn.Linear(self.config.hidden_size, classifier_hidden_size)
                self.classifier = nn.Sequential(self.activation,
    ...
    def forward(
        ...
        return out[:,1:2], x, ecg_attentions

class my_ecg_config:
    def __init__(self):
        self.load_path = './models/pretrained/hubert_ecg_finetuned.pt'
        #'./HuBERT-ECG-master/models_497/hubert_3_iteration_39_finetuned_loss_0.9048507213592529_0.8013468384742737_0.7975113391876221.pt'
...
class SpaceTimeFactorizedViViT_ECG(nn.Module):
    def __init__(
        ...
        if mode == "ef":
            ...
            self.global_value_output_mlp_1 = nn.Sequential(
                nn.Linear(vid_hidden_size, vid_hidden_size // 2),
                nn.ReLU(inplace=True),
                nn.Dropout(p=output_dropout_rate)
            )

            self.global_value_output_mlp_2 = nn.Sequential(
                nn.Linear(vid_hidden_size // 2 + 16, vid_hidden_size // 4),
                nn.ReLU(inplace=True),
                nn.Dropout(p=output_dropout_rate),
                nn.Linear(vid_hidden_size // 4, 1),
            )
        ...
        self.ecg_model.load_state_dict(checkpoint['model_state_dict'], strict=False) # load backbone weights

        self.video_backbone = backbone
        self.avg_pool = nn.AdaptiveAvgPool3d((1, 1, 1))
        if self.fuse_mode != 'global':
            self.gnn_model = fine_grained_lge_value.MultiViewLGEModel(hidden_dim=768, num_views=5, fuse_mode=fuse_mode, loc=loc)
        else:
            self.gnn_model = fine_grained_lge_value.MultiViewLGEModel(hidden_dim=640, num_views=5, fuse_mode=fuse_mode, loc=loc)
    def forward(self, data_dict):
        ...
        x, patches, _ = self.video_backbone(x)
        if len(x.shape) == 2:
            x = x.view(bs, nvid, -1)
        else:
        ...
        if self.fuse_mode != 'global':
            # fine-grained lge value prediction
            patches = patches.contiguous().view(
                bs, nvid, patches.shape[1], patches.shape[2])
            patch_mask = vid_mask.unsqueeze(2).unsqueeze(3).unsqueeze(4).expand(-1, -1, 8, 7, 7)
            patch_mask = patch_mask.view(patch_mask.shape[0], patch_mask.shape[1], -1).permute(0, 2, 1).contiguous().view(patches.shape[0], -1)
            lge_value_bfs, lge_loc, segmental_attn_weights = self.gnn_model(patches, patch_mask)
            lge_value = F.sigmoid(lge_value_bfs) * 100
        else:
            global_fea = x.squeeze(-1).view(bs, -1)
            lge_value_bfs, lge_loc, segmental_attn_weights = self.gnn_model(global_fea)
            lge_value = F.sigmoid(lge_value_bfs) * 100
        ...
        if self.output_mlp is not None:
            ...
            x = self.global_value_output_mlp_1(x)
            x = torch.cat((x, lge_value_bfs),dim=-1)
            global_value = self.global_value_output_mlp_2(x)
            global_value = F.sigmoid(global_value) * 100
        ...
        return {
            ...
            "lge_value": lge_value,
            "lge_loc":lge_loc,
            "segmental_attn_weights":segmental_attn_weights,
...
class PPNet(nn.Module):
    ...
    def get_PPC_loss(
        ...
        print(proto_indices)
        proto_indices = proto_indices.to(torch.int64)
